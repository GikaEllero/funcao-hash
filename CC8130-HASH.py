...
def EncontraErro(frase1, frase2):
    indices = []
...
sha2 = ["d24de3ec3835115c576a55188a31761b73af93ed2c45a171c810bb66b24b08f9",
...
for i in range (len(frases)):
    SHA256 = hashlib.sha256(frases[i].encode('utf-8')).hexdigest()
    MD5 = hashlib.md5(frases[i].encode('utf-8')).hexdigest()

    print("Frase: ", frases[i])
    if sha2[i] == SHA256:
        print("O hash SHA256 está correto: ", SHA256)
    else:
        print("O hash SHA256 está errado! Segue correto: ", SHA256)
        erro = EncontraErro(sha2[i], SHA256)
        print(f"O erro está nos indices {erro}")

    if md5[i] == MD5:
        print("O hash MD5 está correto: ", MD5)
    else:
        print("O hash MD5 está errado! Segue correto: ", MD5)
        erro = EncontraErro(md5[i], MD5)
        print(f"O erro está nos indices {erro}")
